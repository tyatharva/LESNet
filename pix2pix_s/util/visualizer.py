...
def safe_fss(pred, target, threshold, window):
    pred_exceed = (pred >= threshold).sum()
    target_exceed = (target >= threshold).sum()
    if pred_exceed == 0 and target_exceed == 0: return 1.0  # Perfect match, no precipitation above threshold
    score = fss(pred, target, threshold, window)
    if np.isnan(score): return 0.0  # Fallback if NaN occurs for other reasons
    return score


def combined_metric(pred, target):
    ...
    if np.std(pred) * np.std(target) != 0:
        ...
        max_val = max(target_t.max().item(), pred_t.max().item())
        ssim_value = piq.multi_scale_ssim(pred_t, target_t, data_range=max_val).numpy()
    fss_25_5 = safe_fss(pred, target, 0.25, 5)
    fss_1_5 = safe_fss(pred, target, 1, 5)
    fss_2_5 = safe_fss(pred, target, 2, 5)
    combined = 0.1*ssim_value + 0.3*fss_25_5 + 0.3*fss_1_5 + 0.3*fss_2_5
...
def compute_metrics_full(pred, target):
    ...
    fss_1_5 = safe_fss(pred, target, 1, 5)
    fss_2_5 = safe_fss(pred, target, 2, 5)
    return {'MSE': mse, 'MAE': mae, 'PCC': pcc, 'MS-SSIM': ssim_value,
...
def save_images(phase, full, webpage, visuals, image_path, aspect_ratio=1.0, width=256, use_wandb=False):
    ...
    if phase == 'val':
        if full:
            ...
            model_metrics = combined_metric(pred, target)
            hrrr_metrics = combined_metric(hrrr, target)
    else:
        ds = xr.open_dataset(f"../dataset/{smallname[-1]}/hrrr/{smallname}_hrrr.zarr")
        hrrr = np.flip(ds['APCP_surface'].values, axis=0).copy()
        ds.close()
        model_metrics = compute_metrics_full(pred, target)
        hrrr_metrics = compute_metrics_full(hrrr, target)
    ...
    if phase not in ["train", "val", "test"]:
        ...
        ds["Target"].attrs["description"] = "MRMS target"
